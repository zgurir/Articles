@props(['comment', 'i'])
<div class="w-full flex-col justify-start items-start gap-8 flex" style="margin-left: calc({{$i}}vh)"> 
@foreach ($comment->replies as $reply)


                    <div class="w-full pb-6 border-b border-gray-300 justify-start items-start gap-2.5 inline-flex">
                        <img class="w-10 h-10 rounded-full object-cover" src="{{ Gravatar::get($comment->user->email) }}" alt="Mia Thompson image" />
                        <div class="w-full flex-col justify-start items-start gap-3.5 inline-flex">
                            <div class="w-full justify-start items-start flex-col flex gap-1">
                                <div class="w-full justify-between items-start gap-1 inline-flex">
                                    <h5 class="text-gray-900 text-sm font-semibold leading-snug">{{ $reply->user->name }}</h5>
                                    <span class="text-right text-gray-500 text-xs font-normal leading-5">{{$reply->created_at->diffForHumans()}}</span>
                                </div> 
                                <h5 class="text-gray-800 text-sm font-normal leading-snug">{{ $reply->content }}</h5>
                            </div>
